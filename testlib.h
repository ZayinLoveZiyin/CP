...
class random_t {
 ...
 public:
  ...
  double next(double from, double to) {
    if (from >= to)
      __testlib_fail(
          "random_t::next(double from, double to): from should be strictly "
          "less than to");
  ...
  double wnext(double from, double to, int type) {
    if (from >= to)
      __testlib_fail(
          "random_t::wnext(double from, double to, int type): from should be "
  ...
  std::vector<T> partition(int size, T sum, T min_part) {
    ...
    if (*std::min_element(result.begin(), result.end()) < min_part)
      __testlib_fail(
          "random_t::partition: partition min is expected to be no less than "
          "the given min_part");
  ...
  void writeTestOverviewLog() {
    if (!_testOverviewLogFileName.empty()) {
      ...
      if (fileName == "stdout")
        ...
        if (NULL == f)
          __testlib_fail(
              "Validator::writeTestOverviewLog: can't write test overview log "
              "to (" +
              fileName + ")");
      }
  ...
  ~TestlibScorerGuard() {
    if (testlibMode == _scorer) {
      ...
      while (!inf.eof()) {
        ...
        if (!line.empty()) testResults.push_back(deserializeTestResult(line));
      }
      inf.readEof();
